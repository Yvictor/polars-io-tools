...
impl<S: AsyncRead + AsyncWrite + Unpin + Send> Client<S> {
    ...
    /// Executes a query and decodes cells directly into a row sink.
    pub async fn query_into<'a, 'b, T>(
        &'a mut self,
        query: impl Into<Cow<'b, str>>,
        params: &'b [&'b dyn ToSql],
        sink: &mut T,
        ...
        T: crate::RowSink,
    {
        self.connection.flush_stream().await?;
        let rpc_params = Self::rpc_params(query);
        let params = params.iter().map(|parameter| parameter.to_sql());
        self.rpc_perform_query(RpcProcId::ExecuteSQL, rpc_params, params)
            .await?;
        TokenStream::new(&mut self.connection).consume_into(sink).await
    }
