...
/// Receives decoded cells without materializing an intermediate [`Row`].
pub trait RowSink {
    /// Called immediately before the first cell of a row.
    fn start_row(&mut self) -> Result<(), String>;
    /// Receives one decoded cell in wire order.
    fn append_cell(&mut self, index: usize, value: ColumnData<'static>) -> Result<(), String>;
    /// Called after the final cell of a row.
...
impl Row {
    ...
    /// Decoded values in column order.
    pub fn values(&self) -> &[ColumnData<'static>] {
        self.data.as_slice()
    }
