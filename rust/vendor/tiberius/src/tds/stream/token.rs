...
use crate::{
    ...
    Error, RowSink, SqlReadBytes, TokenType,
};
use futures_util::{
...
{
    pub(crate) async fn consume_into<T: RowSink>(mut self, sink: &mut T) -> crate::Result<()> {
        while !self.conn.is_eof() {
            ...
            match ty {
                TokenType::Row => TokenRow::decode_into(self.conn, sink).await?,
                TokenType::NbcRow => TokenRow::decode_nbc_into(self.conn, sink).await?,
                ...
                _ => return Err(Error::Protocol(format!("token {ty:?} is not supported").into())),
            }
        }
