    ...
    R: SqlReadBytes + Unpin,
{
    ...
    match (data, ty) {
        ...
        (Some(buf), BigChar) | (Some(buf), BigVarChar) => {
            if buf.is_ascii() {
                // ASCII is valid UTF-8 in every supported SQL Server codepage.
                // Reuse the wire allocation instead of decoding into a second
                // String allocation.
                return Ok(Some(unsafe { String::from_utf8_unchecked(buf) }.into()));
            }
