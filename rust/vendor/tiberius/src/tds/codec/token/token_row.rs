...
use crate::{tds::codec::ColumnData, BytesMutWithTypeInfo, RowSink, SqlReadBytes, TokenType};
use bytes::BufMut;
...
impl<'row, 'bytes, 'columns> Encode<BytesMutWithDataColumns<'bytes, 'columns>>
    for TokenRow<'row>
{
    fn encode(
        self,
        dst: &mut BytesMutWithDataColumns<'bytes, 'columns>,
    ) -> crate::Result<()> {
...
impl<'a> TokenRow<'a> {
    ...
    /// Returns the decoded column values in wire order.
    pub fn as_slice(&self) -> &[ColumnData<'a>] {
        &self.data
    }
...
impl TokenRow<'static> {
    pub(crate) async fn decode_into<R, S>(src: &mut R, sink: &mut S) -> crate::Result<()>
    where
        R: SqlReadBytes + Unpin,
        S: RowSink,
    {
        let col_meta = src.context().last_meta().unwrap();
        sink.start_row().map_err(|error| crate::Error::Conversion(error.into()))?;
        for (index, column) in col_meta.columns.iter().enumerate() {
            let data = ColumnData::decode(src, &column.base.ty).await?;
            sink.append_cell(index, data)
                .map_err(|error| crate::Error::Conversion(error.into()))?;
        }
        sink.finish_row()
            .map_err(|error| crate::Error::Conversion(error.into()))
    }

    pub(crate) async fn decode_nbc_into<R, S>(src: &mut R, sink: &mut S) -> crate::Result<()>
    where
        R: SqlReadBytes + Unpin,
        S: RowSink,
    {
        ...
        sink.start_row().map_err(|error| crate::Error::Conversion(error.into()))?;
        for (index, column) in col_meta.columns.iter().enumerate() {
            let data = if row_bitmap.is_null(index) {
